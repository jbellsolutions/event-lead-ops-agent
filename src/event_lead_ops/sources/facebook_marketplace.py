...
_ID_PATTERN = re.compile(r"/marketplace/item/(\d+)")
_PRICE_PATTERN = re.compile(r"(?:US\$|\$)?\s*([0-9][0-9,]*(?:\.[0-9]{1,2})?)")
...
def listing_id_from_url(value: str) -> str:
    ...
    if not match:
        raise ValueError("Marketplace URL does not contain a listing ID")
...
def normalize_listing(raw: dict[str, Any]) -> SourceRecord:
    ...
    return SourceRecord(
        ...
        metadata={
            ...
            "seller_alias": raw.get("seller_alias"),
        },
